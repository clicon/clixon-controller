#!/usr/bin/env bash
# Test exclusive lock
# See  https://github.com/clicon/clixon-controller/issues/92 and
# https://github.com/clicon/clixon-controller/issues/204
# Assume two users, one admin (clicon) and one limited (whoami)
# With CLICON_AUTOLOCK, NACM and mini-service
# 
# The tests are:
# 1) admin makes edits of protected data in openconfig-interfaces
# 2) limited cannot see protected data with show config
# 3) limited cannot see protected data with show compare
# 4) limited cannot see protected data with commit diff
# 5) limited cannot make commit
# 6) admin makes a service commit triggering pyapi and changes device data
# 7) admin can do commit diff
# 8) admin can do commit
# ? Check limited cannot?
# 9) Configuration on devices is updated
#
# (3) and (6) does not work 2025-05-30
# XXX expand does not work for limited after show devices device ?

# Magic line must be first in script (see README.md)
s="$_" ; . ./lib.sh || if [ "$s" = $0 ]; then exit 0; else return 0; fi

# Debug early exit
: ${early:=false}
>&2 echo "early=true for debug "

: ${check:=false}

# Reset devices with initial config
(. ./reset-devices.sh)

dir=/var/tmp/$0
modules=$dir/modules
fyang=$dir/example.yang
CFG=$dir/controller.xml
CFD=$dir/conf.d
test -d $dir || mkdir -p $dir
test -d $modules || mkdir -p $modules
test -d $CFD || mkdir -p $CFD
pycode=$modules/example.py

# Use two users
ADMIN=${CLICON_USER} # Clicon user
LIMITED=$(whoami)    # Who runs the script

if [ "$ADMIN" = "${LIMITED}" ]; then
    echo "Test requires whoami != CLICON_USER"
    if [ "$s" = $0 ]; then exit 0; else return 0; fi
fi

# Common NACM scripts
. ./nacm.sh

cat <<EOF > $CFG
<clixon-config xmlns="http://clicon.org/config">
  <CLICON_FEATURE>ietf-netconf:startup</CLICON_FEATURE>
  <CLICON_FEATURE>clixon-restconf:allow-auth-none</CLICON_FEATURE>
  <CLICON_FEATURE>clixon-restconf:fcgi</CLICON_FEATURE>
  <CLICON_CONFIGFILE>$CFG</CLICON_CONFIGFILE>
  <!--CLICON_CONFIGDIR>$CFD</CLICON_CONFIGDIR-->
  <CLICON_CONFIG_EXTEND>clixon-controller-config</CLICON_CONFIG_EXTEND>
  <CLICON_YANG_DIR>${DATADIR}/clixon</CLICON_YANG_DIR>
  <CLICON_YANG_DIR>${DATADIR}/controller/common</CLICON_YANG_DIR>
  <CLICON_YANG_DIR>${DATADIR}/controller/main</CLICON_YANG_DIR>
  <CLICON_YANG_MAIN_DIR>$dir</CLICON_YANG_MAIN_DIR>
  <CLICON_YANG_DOMAIN_DIR>$dir</CLICON_YANG_DOMAIN_DIR>
  <CLICON_YANG_SCHEMA_MOUNT>true</CLICON_YANG_SCHEMA_MOUNT>
  <CLICON_YANG_SCHEMA_MOUNT_SHARE>true</CLICON_YANG_SCHEMA_MOUNT_SHARE>
  <CLICON_YANG_USE_ORIGINAL>true</CLICON_YANG_USE_ORIGINAL>
  <CLICON_BACKEND_DIR>${LIBDIR}/controller/backend</CLICON_BACKEND_DIR>
  <CLICON_BACKEND_USER>${ADMIN}</CLICON_BACKEND_USER>
  <CLICON_BACKEND_PRIVILEGES>none</CLICON_BACKEND_PRIVILEGES>
  <CLICON_BACKEND_PIDFILE>${LOCALSTATEDIR}/run/controller/controller.pid</CLICON_BACKEND_PIDFILE>
  <CLICON_RESTCONF_INSTALLDIR>${SBINDIR}</CLICON_RESTCONF_INSTALLDIR>
  <CLICON_RESTCONF_PRIVILEGES>drop_perm</CLICON_RESTCONF_PRIVILEGES>
  <CLICON_CLI_DIR>${LIBDIR}/controller/cli</CLICON_CLI_DIR>
  <CLICON_CLISPEC_DIR>${LIBDIR}/controller/clispec</CLICON_CLISPEC_DIR>
  <CLICON_CLI_MODE>operation</CLICON_CLI_MODE>
  <CLICON_CLI_HELPSTRING_TRUNCATE>true</CLICON_CLI_HELPSTRING_TRUNCATE>
  <CLICON_CLI_HELPSTRING_LINES>1</CLICON_CLI_HELPSTRING_LINES>
  <CLICON_CLI_OUTPUT_FORMAT>text</CLICON_CLI_OUTPUT_FORMAT>
  <CLICON_SOCK>${LOCALSTATEDIR}/run/controller/controller.sock</CLICON_SOCK>
  <CLICON_SOCK_GROUP>${CLICON_GROUP}</CLICON_SOCK_GROUP>
  <CLICON_SOCK_PRIO>true</CLICON_SOCK_PRIO>
  <CLICON_AUTOLOCK>true</CLICON_AUTOLOCK>
  <CLICON_XMLDB_DIR>$dir</CLICON_XMLDB_DIR>
  <CLICON_XMLDB_MULTI>true</CLICON_XMLDB_MULTI>
  <CLICON_STARTUP_MODE>init</CLICON_STARTUP_MODE>
  <CLICON_NACM_MODE>internal</CLICON_NACM_MODE>
  <CLICON_NACM_CREDENTIALS>except</CLICON_NACM_CREDENTIALS>
  <CLICON_NACM_DISABLED_ON_EMPTY>true</CLICON_NACM_DISABLED_ON_EMPTY>
  <CLICON_STREAM_DISCOVERY_RFC5277>true</CLICON_STREAM_DISCOVERY_RFC5277>
  <CLICON_VALIDATE_STATE_XML>true</CLICON_VALIDATE_STATE_XML>
  <!-- Cant split config file since pyapi cant read that -->
  <CONTROLLER_ACTION_COMMAND xmlns="http://clicon.org/controller-config">${BINDIR}/clixon_server.py -f $CFG</CONTROLLER_ACTION_COMMAND>
  <CONTROLLER_PYAPI_MODULE_PATH xmlns="http://clicon.org/controller-config">$modules</CONTROLLER_PYAPI_MODULE_PATH>
  <CONTROLLER_PYAPI_MODULE_FILTER xmlns="http://clicon.org/controller-config"></CONTROLLER_PYAPI_MODULE_FILTER>
  <CONTROLLER_PYAPI_PIDFILE xmlns="http://clicon.org/controller-config">/tmp/clixon_pyapi.pid</CONTROLLER_PYAPI_PIDFILE>  <!-- clicon goup cannot write in ${LOCALSTATEDIR} -->
</clixon-config>
EOF

cat <<EOF > $fyang
module example {
    namespace "http://clicon.org/example";
    prefix ex;

    import clixon-controller {
        prefix ctrl;
    }
    revision 2025-05-17{
        description "TNSR service example";
    }
    augment "/ctrl:services" {
        list example {
            key service-name;
            leaf service-name {
                type string;
            }
            description "TNSR example service";
            uses ctrl:created-by-service;
        }
    }
}
EOF

cat <<EOF > $pycode
SERVICE = "example"

def setup(root, log, **kwargs):
    try:
        _ = root.services
    except Exception:
        return
    for instance in root.services.example:
        if instance.service_name != kwargs["instance"]:
            continue
        description = instance.service_name.get_data()
        for device in root.devices.device:
            device.config.system.config.create("login-banner", data=description)
EOF


RULES=$(cat <<EOF
   <nacm xmlns="urn:ietf:params:xml:ns:yang:ietf-netconf-acm">
     <enable-nacm>true</enable-nacm>
     <!--read-default>permit</read-default-->
     <read-default>deny</read-default>
     <write-default>deny</write-default>
     <exec-default>deny</exec-default>

     <groups>
       <group>
         <name>admin</name>
         <user-name>root</user-name>
         <user-name>${ADMIN}</user-name>
       </group>
       <group>
         <name>limited</name>
         <user-name>$LIMITED</user-name>
         
       </group>
     </groups>

     $NADMIN

     <rule-list>
       <name>limited-acl</name>
       <group>limited</group>
       <rule>
         <name>permit-ops</name>
         <comment>Allow all operations</comment>
         <rpc-name>*</rpc-name>
         <module-name>*</module-name>
         <access-operations>exec</access-operations>
         <action>permit</action>
       </rule>
       <rule>
         <name>permit-read-path</name>
         <comment>Allow some reads</comment>
         <module-name>*</module-name>
         <access-operations>read</access-operations>
         <path xmlns:ctrl="urn:example:controller" xmlns:oc-sys="http://openconfig.net/yang/system">/ctrl:devices/ctrl:device/ctrl:config/oc-sys:system</path>
         <action>permit</action>
       </rule>
     </rule-list>
   </nacm>
EOF
)

# Set initial NACM rules in startup enabling admin and a single param config
sudo rm -rf $dir/startup.d/*
cat <<EOF > $dir/startup_db
<config>
  ${RULES}
  <devices xmlns="http://clicon.org/controller">
EOF

ii=1
for ip in $CONTAINERS; do
    NAME="$IMG$ii"
    cat <<EOF >> $dir/startup_db    
    <device>
      <name>$NAME</name>
      <enabled>true</enabled>
      <user>$USER</user>
      <conn-type>NETCONF_SSH</conn-type>
      <addr>$ip</addr>
    </device>
EOF
    ii=$((ii+1))
done

cat <<EOF >> $dir/startup_db
  </devices>
</config>
EOF

# Sleep and verify devices are open
function sleep_open()
{
    pre=$1
    for j in $(seq 1 10); do
        new "Verify devices are open"
        ret=$($pre $clixon_cli -1 -f $CFG show connections)
        match1=$(echo "$ret" | grep --null -Eo "openconfig1.*OPEN") || true
        match2=$(echo "$ret" | grep --null -Eo "openconfig2.*OPEN") || true
        if [ -n "$match1" -a -n "$match2" ]; then
            break;
        fi
        echo "retry after sleep"
        sleep 1
    done
    if [ $j -eq 10 ]; then
        err "device openconfig OPEN" "Timeout"
    fi
}

if $BE; then
    new "Kill old backend"
    stop_backend -f $CFG

    new "Start new backend -s startup -f $CFG"
    start_backend -s startup -f $CFG
fi

new "Wait backend"
wait_backend

new "connection open"
expectpart "$(sudo -u ${CLICON_USER} $clixon_cli -1 -f $CFG -E $CFD connection open)" 0 "^$"

new "Sleep and verify devices are open"
sleep_open "sudo -u ${CLICON_USER}"

if ${early}; then
    exit # for starting controller with devices and debug
fi

new "Spawn expect script"

sudo expect - "$clixon_cli" "$CFG" "$ADMIN" "$LIMITED" <<'EOF'
# Use of expect to start parallel cli:s

set clixon_cli [lindex $argv 0]
set CFG [lindex $argv 1]
set ADMIN [lindex $argv 2]
set LIMITED [lindex $argv 3]

puts "Test Expect lock-exclusive"

spawn sudo -u $ADMIN $clixon_cli -f $CFG -m configure -- -e
set cli1 $spawn_id

spawn sudo -u $LIMITED $clixon_cli -f $CFG -m configure -- -e
set cli2 $spawn_id

puts "Test: 1) admin makes edits of protected data in openconfig-interfaces (name)"
send -i $cli1 "set devices device openconfig1 config interfaces interface e0 config name e0\n"
expect ""

puts "Test: 1) admin makes edits of protected data in openconfig-interfaces (type)"
send -i $cli1 "set devices device openconfig1 config interfaces interface e0 config type ianaift:x213\n"
expect ""

puts "Test: 2) limited cannot see protected data with show config"
send -i $cli2 "show devices device openconfig1 config\n"
expect ""

puts "Test: 3) limited cannot see protected data with show compare"
send -i $cli2 "show compare\n"
expect ""

puts "Test: 4) limited cannot see protected data with commit diff"
send -i $cli2 "commit diff\n"
expect ""

puts "Test: 5) limited cannot make commit"
send -i $cli2 "commit\n"
expect ""

puts "Test: 6) admin makes a service commit triggering pyapi and changes device data"
send -i $cli1 "set service example Mybanner\n"
expect "";

puts "Test: 7) admin can do commit diff"
send -i $cli1 "commit diff\n"
expect "";

puts "Test: 8) admin can do commit"
send -i $cli1 "commit\n"
expect "";

EOF
ret=$?
echo "ret:$ret"
if [ $ret -ne 0 ]; then
    err1 "Failed: test lock exclusive using expect"
fi

# Show the configuration on the devices after mini-service using SSH
new "9) Configuration on devices is updated"
for container in $CONTAINERS; do
    new "Verify configuration on $container"
    expectpart "$(ssh ${SSHID} -l $USER $container clixon_cli -1 show configuration cli)" 0 "system config login-banner Mybanner"
done


if $BE; then
    new "Kill old backend"
    stop_backend -f $CFG
fi

endtest
exit


trap cleanup SIGINT SIGTERM EXIT

# Create background admin cli
cli1_in="/tmp/cli1_in_$$"
cli1_out="/tmp/cli1_out_$$"
#cli1_out="$dir/cli1_out_$$"
#cli1_out="/dev/null"

mkfifo $cli1_in $cli1_out

# Create admin process
sudo -u ${CLICON_USER} clixon_cli -f $CFG -m configure -- -e < $cli1_in &
cli1_pid=$!

new "1. admin makes edits of protected data"
write_to_process "set devices device openconfig1 config interfaces interface e0 config name e0" $cli1_in

write_to_process "set devices device openconfig1 config interfaces interface e0 config type ianaift:x213" $cli1_in

#new "Check reading process"
#ret=$(cat < $cli1_out)




sleep 100

new "limited does not see protected data: show config"

# NOTE: This works OK, ie it does not see
expectpart "$($clixon_cli -1 -m configure -f $CFG show devices device openconfig1 config)" 0 "hostname openconfig1" --not-- "openconfig-interfaces:interfaces"

# XXX NOTE: This does NOT work, ie it sees"
new "limited does not see protected data: show compare"
expectpart "$($clixon_cli -1 -m configure -f $CFG show compare)" 0 kalle --not-- "+           openconfig-interfaces:interfaces {"


new "2. limited does not see protected data: commit diff"
# NOTE: This actually seems to work (dont know why)"
expectpart "$($clixon_cli -1 -f $CFG -m configure commit diff)" 0 "^$" --not-- "\+  <interfaces xmlns=\"http://openconfig.net/yang/interfaces\">"

new "write admin sees protected data: show compare"
write_to_process "show compare" $cli1_in

new "write admin sees protected data: commit diff"
write_to_process "commit diff" $cli1_in

new "cat candidate"
sudo cat /var/tmp/test-lock-exclusive.sh/candidate.d/35819a66b93010f544366902daedc2ed520ead1d.xml



new "cat candidate 2"
sudo cat /var/tmp/test-lock-exclusive.sh/candidate.d/35819a66b93010f544366902daedc2ed520ead1d.xml

if true; then # XXX
new "try again 2"
echo "$clixon_cli -1 -m configure -f $CFG show compare -- -e"
$clixon_cli -1 -m configure -f $CFG show compare -- -e

fi # XXX

new "check admin sees protected data: show compare"
match=$(echo "$ret" | grep --null -Eo "\+           openconfig-interfaces:interfaces {") || true
if [ -z "$match" ]; then
    err "+ openconfig-interfaces:interfaces" "Error: $ret"
fi

new "check admin sees protected data: commit diff"
match=$(echo "$ret" | grep --null -Eo "\++  <interfaces xmlns=\"http://openconfig.net/yang/interfaces\">") || true
if [ -z "$match" ]; then
    err "+ <interfaces>" "Error: $ret"
fi

echo "ret:$ret"
if true; then # XXX
new "try again 3"
echo "$clixon_cli -1 -m configure -f $CFG show compare -- -e"
$clixon_cli -1 -m configure -f $CFG show compare -- -e

fi # XXX

exit

new "3) limited cannot make commit"
expectpart "$($clixon_cli -1 -m configure -f $CFG commit)" 0 "^$" # XXX

new "Check config"
expectpart "$($clixon_cli -1 -f $CFG show config devices device openconfig1 config interfaces)" 0 kalle

echo "ret:$ret"


exit

new "3. limited cannot make commit"
expectpart "$($clixon_cli -1 -f $CFG -m configure commit)" 255 ""



if $BE; then
    new "Kill old backend"
    stop_backend -f $CFG
fi

endtest
