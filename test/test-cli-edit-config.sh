...
function testrun()
{
    ...
    new "Show devices ${IMG}* diff, should be empty"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD 'show devices ${IMG}* diff')" 0 ""

    new "Show devices ${IMG}1 diff, should be empty"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD show devices ${IMG}1 diff)" 0 ""

    new "Show devices ${IMG}2 diff, should be empty"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD show devices ${IMG}2 diff)" 0 ""
    ...
    new "Verify show compare on ${IMG}1"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure -o CLICON_CLI_OUTPUT_FORMAT=text show compare)" 0 "^-\ *hostname \"${IMG}1\";" "^+\ *hostname \"test1\";"

    new "Verify commit diff on ${IMG}1"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure commit diff)" 0 "^\-\ *<hostname>${IMG}1</hostname>" "^+\ *<hostname>test1</hostname>"

    new "Rollback hostname on ${IMG}1"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure rollback)" 0 ""

    new "Configure hostnames on ${IMG}*"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure "set devices device ${IMG}* config system config hostname test")" 0 ""

    new "Verify show compare on ${IMG}*"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure -o CLICON_CLI_OUTPUT_FORMAT=text show compare)" 0 "^\-\ *hostname \"${IMG}1\";" "^+\ *hostname \"test\";" "^\-\ *hostname \"${IMG}2\";"

    new "Verify commit diff on ${IMG}*"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure commit diff)" 0 "^\-\ *<hostname>${IMG}1</hostname>" "^+\ *<hostname>test</hostname>" "^\-\ *<hostname>${IMG}2</hostname>"

    new "Rollback hostnames on ${IMG}*"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure rollback)" 0 ""

    new "Set hostname to test on ${IMG}*"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure "set devices device ${IMG}* config system config hostname test")" 0 ""
    ...
    for i in $(seq 1 $nr); do
        new "Edit hostname on ${IMG}$i"
        expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure set devices device ${IMG}$i config system config hostname ${IMG}$i)" 0 ""

        new "Commit hostname on ${IMG}$i"
        expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure commit)" 0 ""
    done

    for container in $CONTAINERS; do
        new "Verify hostname on $container"
        expectpart "$(ssh  ${SSHID} -l $USER $container clixon_cli -1 show configuration cli)" 0 "system config hostname ${IMG}*"
    done
    ...
    new "Configure interface xintf on ${IMG}1"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure set devices device ${IMG}1 config interfaces interface xintf config name xintf)" 0 ""

    new "Completion of interfaces"
    expectpart "$(echo "set devices device ${IMG}1 config interfaces interface ?" | $clixon_cli -f $CFG -E $CFD -m configure 2> /dev/null)" 0 "<name>" "xintf"

    new "Delete interface"
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD  -m configure delete devices device ${IMG}1 config interfaces interface xintf)" 0 ""
    ...
    expectpart "$($clixon_cli -1 -f $CFG -E $CFD -m configure set devices device ${IMG}1 config interfaces interface kaka)" 0 ""
...
new "Ensure closed"
expectpart "$($clixon_cli -1 -f $CFG -E $CFD show connections)" 0 "${IMG}1.*CLOSED" "${IMG}2.*CLOSED"
