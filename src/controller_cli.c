              ...
              cxobj       **xylibp)
{
    ...
    if (xml_nsctx_add(nsc, "yanglib", "urn:ietf:params:xml:ns:yang:ietf-yang-library") < 0)
        goto done;
#if 0 // XXX Should be OK, but code in other places break, see test-yang-domain.sh
    ...
    if ((xmodset = xpath_first(xt, nsc, "%s", cbuf_get(cb))) == NULL)
        goto skip;
    if (clixon_xml_parse_string("<yang-library xmlns=\"urn:ietf:params:xml:ns:yang:ietf-yang-library\"/>", YB_NONE, NULL, &xylib, NULL) < 0)
        goto done;
    if (xml_addsub(xylib, xmodset) < 0)
        goto done;
