...
#
# This script is used to build Debian packages for Clixon controller.
#

# Make sure the script is started from the cligen directory
if [ ! -f scripts/version.sh ]; then
    echo "This script must be run from the clixon controller directory."
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to determine the version of Clixon controller."
    exit 1
fi
...
echo -n "clixon-controller (${VERSION}) " > build/debian/changelog
...
# Build Clixon controller and install it to the build/ directory
./configure && make clean && make && sudo make install DESTDIR=`pwd`/build/
